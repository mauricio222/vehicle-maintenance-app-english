...
class TestVehicleManagement:
    ...
    def test_add_vehicle(self, authenticated_page: Page, test_vehicle_data):
        ...
        authenticated_page.wait_for_selector("#nuevoVehiculoModal", state="hidden", timeout=10000)

        # Verify vehicle appears in the list (using alias)
    ...
    def test_view_vehicle_details(self, authenticated_page: Page, test_vehicle_data):
        ...
        # Click on the vehicle to view maintenance - use "View Maintenance" button instead
...
class TestMaintenanceTracking:
    ...
    def test_add_maintenance_record(self, authenticated_page: Page, test_vehicle_data):
        ...
        # Select the vehicle from dropdown - updated selector
        ...
        authenticated_page.wait_for_timeout(1000)  # Give time for options to load
    ...
    def test_ai_suggestions(self, authenticated_page: Page, test_vehicle_data):
        ...
        # Select vehicle
        vehicle_select = authenticated_page.locator("select#vehiculo")
        vehicle_select.wait_for(state="visible")
        authenticated_page.wait_for_timeout(1000)
        ...
        if vehicle_select.locator("option").count() > 1:
            ...
            # Click AI suggestion button if available - check for various button texts
            suggest_button = authenticated_page.locator("button").filter(has_text="Suggest").or_(
                authenticated_page.locator("button").filter(has_text="Sugerir")
            ).or_(
                authenticated_page.locator("button.btn-info")
            )
            ...
            if suggest_button.count() > 0:
                ...
                # Check if suggestion appeared (various possible IDs)
                suggestion_element = authenticated_page.locator(
                    "#tipoSugerencia, #categoriaSugerencia, #maintenanceSuggestion, #categorySuggestion, .suggestion-text"
                )
                if suggestion_element.count() > 0:
                    # Just verify element exists and is not empty
                    expect(suggestion_element.first).to_be_visible()
...
class TestCompleteUserJourney:
    ...
    def test_complete_workflow(self, page: Page):
        ...
        # Wait for modal to close
        page.wait_for_selector("#nuevoVehiculoModal", state="hidden", timeout=10000)
